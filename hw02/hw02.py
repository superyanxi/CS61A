...
def num_sevens(n):
    ...
    if n < 10:
        ...
        if last == 7:
            return 1+num_sevens(all_but_last)
        else:
            return 0+num_sevens(all_but_last)
    """
    all_but_last= n
    count = 0
...
def pingpong(n):
    ...
    if n == 1:
        ...
        num = (pingpong(n-1)+1*direction(n-1))
        print(num)
        return num
...
def pingpong2(n):
    if n == 1:
        print(1)
        return 1,1
    num,direction = pingpong2(n-1)
    if num_sevens(n-1) > 0 or (n-1)%7 == 0:
        direction = -direction
    print(num+direction)
...
def count_change(amount):
    ...
    def helper(amount):
        ...
        while i < amount:
            i = i*2
        return i//2
    max_change = helper(amount)
    def main(amount,change):
        if amount < 0:
            return 0
        if amount == 0:
            return 1
        if change == 1:
            ...
            return main(amount-change,change)+main(amount,change//2)
    return main(amount,max_change)
